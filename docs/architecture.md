...
│    DB/Redis/Prometheus/K8s/Helm/Runbook → LLM → 报告 │
...
| Agent MCP | `internal/agent/mcp` | 受限只读工具：Prometheus Instant/Range、慢请求/错误、队列、告警、变更、Pod/Event/Deployment/Helm、Pod 日志 |
...
- **Kubernetes API**：Agent 的 Pod/Event/Deployment/Helm/日志工具依赖 Worker 的只读 ServiceAccount；仅允许 `techmind` 命名空间

## SRE Agent 运行闭环

```text
Alertmanager Webhook / 管理员手动触发
        ↓
Redis Stream: ops_tasks → OpsWorker
        ↓
基础取证：慢请求、错误、队列、firing 告警、Prometheus、Kubernetes
        ↓
LLM 规划：最多再选择 5 次受限只读工具调用（可提前 final）
        ↓
最近部署变更 + Runbook/历史报告检索
        ↓
LLM 结构化报告 → ops_report
        ├── alert 触发时关联/复用 incident
        └── 每一次工具调用写入 ops_tool_call，管理端展示证据链
```

Agent 只读访问集群；关闭 Incident 是管理员操作，不会自动修改告警状态或执行任何 Kubernetes 写操作。
