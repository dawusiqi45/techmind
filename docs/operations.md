...
→ 慢请求、错误事件、Redis、Prometheus、变更、Runbook → LLM → ops_report
```

Agent 是只读诊断报告生成器：它不修改集群或业务数据。报告为 `done` 才代表任务成功；`failed` 时优先检查 Worker 日志和 LLM 配置。诊断任务失败会最多重试 3 次，随后进入 `tm:stream:ops_tasks:dead` 死信队列供人工复盘。
