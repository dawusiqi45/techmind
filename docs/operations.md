...
→ 慢请求、错误事件、Redis、Prometheus、Kubernetes、变更、Runbook → LLM → ops_report
                                                                        ↓
...
真实 JWT Secret、LLM API Key、数据库连接和 Alertmanager Webhook Token 不得提交到 Git。非本地模式要求 JWT Secret 至少 32 字符且不能使用默认值：
...
应用 Token 写入 Helm `secrets.alertWebhookToken`；Alertmanager 通过 `credentials_file` 读取 `techmind-alertmanager-webhook` Secret。`deploy/kind/deploy.sh` 会创建该 Secret。LLM Key 写入 `secrets.llmApiKey`，Embedding Key 如需语义检索则写入 `secrets.embeddingApiKey`。
...
export TECHMIND_JWT_SECRET="${TECHMIND_JWT_SECRET:-$(kubectl get secret techmind-secret -n techmind -o jsonpath='{.data.jwt-secret}' | base64 -d)}"
...
- Server/Worker ConfigMap 或 Secret：Helm Pod template 带配置校验和，`helm upgrade` 会自动滚动更新；如手动修改集群 Secret，仍需主动重启相关 Deployment。
