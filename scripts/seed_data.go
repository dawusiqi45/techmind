...
var demoUsers = []User{
	{ID: 10001, Username: "alice", Password: "$2a$10$demo", Email: "alice@techmind.io", Avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=alice", Role: 0, Status: 1, CreatedAt: time.Now().Add(-30 * 24 * time.Hour), UpdatedAt: time.Now()},
	{ID: 10002, Username: "bob", Password: "$2a$10$demo", Email: "bob@techmind.io", Avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=bob", Role: 0, Status: 1, CreatedAt: time.Now().Add(-25 * 24 * time.Hour), UpdatedAt: time.Now()},
	{ID: 10003, Username: "admin", Password: "$2a$10$demo", Email: "admin@techmind.io", Avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=admin", Role: 1, Status: 1, CreatedAt: time.Now().Add(-60 * 24 * time.Hour), UpdatedAt: time.Now()},
}
...
func main() {
	...
	fmt.Println("\n🎉 演示数据种子完成！")
	fmt.Println("提示：用户密码为 bcrypt 哈希，实际登录请使用项目注册接口重新注册。")
}
